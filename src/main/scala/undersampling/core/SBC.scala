...
/** Undersampling Based on Clustering algorithm. Original paper: "Under-Sampling Approaches for Improving Prediction of the
  ...
  */
class SBC(override private[undersampling] val data: Data,
  ...
  def sample(file: Option[String] = None, method: String = "random", m: Double = 1.0, k: Int = 3, numClusters: Int,
    ...
    if (file.isDefined) {
      this.logger.setNames(List("DATA SIZE REDUCTION INFORMATION", "IMBALANCED RATIO", "REDUCTION PERCENTAGE", "KMEANS TIME", "TIME"))
      ...
      this.logger.addMsg("KMEANS TIME", "Elapsed time: %s".format(nanoTimeToString(kMeansTime)))
      // Save the time
